...
taskRouter.get('/', async (req: AuthRequest, res) => {
  try {
    ...
    if (writerId) {
      where.members = { some: { userId: writerId, role: { in: WRITER_ROLES } } };
    }
  ...
  try {
    ...
    if (writerId) {
      await prisma.taskMember.create({
        data: { taskId: task.id, userId: writerId, role: 'WRITER' },
      });
    }

    if (designerId) {
      await prisma.taskMember.create({
        data: { taskId: task.id, userId: designerId, role: 'DESIGNER' },
      });
    }
    ...
    if (materialIds && materialIds.length > 0) {
      for (const materialId of materialIds) {
        await prisma.taskMaterial.create({
          data: { taskId: task.id, materialId },
        });
      }
    }
...
taskRouter.put('/:id/status', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
...
taskRouter.post('/:id/review', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    ...
    await prisma.taskStatusLog.create({
      data: {
        ...
        toStatus: newStatus,
        operatorId: req.user!.id,
        note: `审核${status === 'APPROVED' ? '通过' : '退回'}: ${content}`,
      },
    });
...
taskRouter.post('/:id/assign', requireRole('ADMIN', 'CONTENT_MANAGER'), async (req: AuthRequest, res) => {
  try {
    ...
    if (!userId || !role) {
      return res.status(400).json({ error: '用户和角色不能为空' });
    }
    ...
    if (existing) {
      return res.status(400).json({ error: '该成员已分配此角色' });
    }

    await prisma.taskMember.create({
      data: { taskId: id, userId, role },
    });
